...
const polygonProvider = new ethers.providers.JsonRpcProvider(process.env.RPC_POLYGON)

const EscrowWalletContract = new ethers.Contract(
  EscrowWallet.address[process.env.CHAINID_POLYGON],
  EscrowWallet.abi,
  polygonProvider
);
...
EscrowWalletContract.on('BurnToken', async (user, token, amount_bn, nonce_bn, event) => {
  ...
  try {
    ...
    const TokenContract = new ethers.Contract(tokenAddress, MockSho.abi, polygonProvider)
    ...
    burnEvent = await BurnEvent.findOne({
      nonce: nonce,
      chainId: process.env.CHAINID_POLYGON
    })
